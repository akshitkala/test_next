      ...
      <main className="relative pt-32 pb-20 md:pt-48 md:pb-32">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid gap-16 lg:grid-cols-2 lg:items-center">
            <div>
              <h2 className="mb-4 text-sm font-bold uppercase tracking-widest text-accent">{content.company.company_mission_heading_small}</h2>
              <h1 className="mb-8 text-5xl font-extrabold tracking-tight md:text-7xl">
                Defining the <br />
                <span className="text-gradient">{content.company.company_mission_heading_large_part2}</span>
              </h1>
              <p className="mb-8 text-xl leading-relaxed text-muted-foreground">{content.company.company_mission_paragraph_1}</p>
              <p className="mb-10 text-lg leading-relaxed text-muted-foreground">{content.company.company_mission_paragraph_2}</p>
              
              <div className="flex flex-wrap gap-8">
                <div>
                  <div className="text-4xl font-bold">{content.company.company_stats_requests_value}</div>
                  <div className="text-sm text-muted-foreground uppercase tracking-widest mt-1">{content.company.company_stats_requests_label}</div>
                </div>
                <div>
                  <div className="text-4xl font-bold">{content.company.company_stats_uptime_value}</div>
                  <div className="text-sm text-muted-foreground uppercase tracking-widest mt-1">{content.company.company_stats_uptime_label}</div>
                </div>
                <div>
                  <div className="text-4xl font-bold">{content.company.company_stats_countries_value}</div>
                  <div className="text-sm text-muted-foreground uppercase tracking-widest mt-1">{content.company.company_stats_countries_label}</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              <div className="rounded-3xl bg-white/5 border border-white/10 p-2 aspect-square group overflow-hidden">
                <div className="w-full h-full rounded-[1.25rem] bg-gradient-to-br from-accent/20 to-purple-500/10 flex items-center justify-center transition-transform group-hover:scale-[1.02]">
                  <span className="text-accent font-bold">{content.company.company_culture_card_text}</span>
                </div>
              </div>
