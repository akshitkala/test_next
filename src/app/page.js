...
export default function Home() {
  const stats = [
    ...
    { label: "Acre Campus", value: "600+" },
    { label: "Top Rankings", value: "NIRF #1" },
  ];
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex flex-col items-center text-center">
            ...
            <div className="flex flex-col gap-4 sm:flex-row">
              <button className="group relative flex h-14 items-center justify-center gap-2 overflow-hidden rounded-full bg-gradient-primary px-10 font-semibold text-white transition-all hover:shadow-[0_0_40px_rgba(245,130,32,0.4)] hover:scale-105 active:scale-95">
                Explore Programs
                <svg className="h-5 w-5 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
              </button>
              <button className="flex h-14 items-center justify-center rounded-full border border-white/10 bg-white/5 px-10 font-semibold transition-all hover:bg-white/10 hover:border-white/20">
                Virtual Tour
              </button>
            </div>
            ...
            <div className="mt-24 grid w-full max-w-5xl grid-cols-2 gap-8 md:grid-cols-4">
              {stats.map((stat, i) => (
                <div key={i} className="flex flex-col items-center p-6 rounded-2xl bg-white/5 border border-white/5 hover:border-accent/30 transition-colors group">
            ...
            <div className="mt-40 grid gap-12 lg:grid-cols-2 items-center text-left">
               <div>
                  ...
                  <div className="space-y-6">
                     {[
                        { title: "Global Exposure", desc: "Partnerships with 200+ foreign universities for exchange programs." },
                        { title: "Placement Excellence", desc: "Top MNCs visit every year with record-breaking packages." },
                        { title: "Research Driven", desc: "Dedicated research wings and innovation labs for students." },
                        { title: "Holistic Development", desc: "300+ clubs and societies for extracurricular excellence." }
                     ].map((item, i) => (
                        <div key={i} className="flex gap-6 p-4 rounded-2xl hover:bg-white/5 transition-colors group">
