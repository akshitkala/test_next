        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex flex-col items-center text-center">
            <div className="mb-8 inline-flex animate-float items-center rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-sm font-medium text-accent">
              <span className="mr-2 rounded-full bg-accent/20 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">{content.home.admissions_open_label}</span>{content.home.shape_your_future_label}</div>
            
            <h1 className="mb-8 max-w-5xl text-5xl font-extrabold tracking-tight md:text-7xl lg:text-8xl leading-tight">{content.home.transforming_education_heading}<br />
              <span className="text-gradient">{content.home.empowering_global_leaders_heading}</span>
            </h1>
            
            <p className="mb-12 max-w-3xl text-lg text-muted-foreground md:text-xl leading-relaxed">{content.home.experience_world_class_education_paragraph}</p>

            <div className="flex flex-col gap-4 sm:flex-row">
              <button className="group relative flex h-14 items-center justify-center gap-2 overflow-hidden rounded-full bg-gradient-primary px-10 font-semibold text-white transition-all hover:shadow-[0_0_40px_rgba(245,130,32,0.4)] hover:scale-105 active:scale-95">{content.home.explore_programs_button_label}<svg className="h-5 w-5 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
              </button>
              <button className="flex h-14 items-center justify-center rounded-full border border-white/10 bg-white/5 px-10 font-semibold transition-all hover:bg-white/10 hover:border-white/20">{content.home.virtual_tour_button_label}</button>
            </div>
            ...
            <div className="mt-40 w-full">
              <div className="flex flex-col items-center mb-16">
                ...
                <p className="text-muted-foreground text-center max-w-2xl">{content.home.choose_from_200_programs_paragraph}</p>
              </div>
            ...
            <div className="mt-40 w-full relative">
               ...
               <div className="relative p-12 md:p-20 rounded-[3rem] border border-white/10 bg-white/5 overflow-hidden">
                  <div className="max-w-2xl">
                     <h2 className="text-4xl md:text-5xl font-bold mb-6">{content.home.ready_to_start_your_journey_heading}</h2>
                     <p className="text-xl text-muted-foreground mb-10">{content.home.join_thousands_of_students_paragraph}</p>
                     <div className="flex flex-col sm:flex-row gap-4">
                        <button className="h-14 px-10 rounded-full bg-accent text-white font-bold hover:shadow-[0_0_30px_rgba(245,130,32,0.5)] transition-all">{content.home.apply_now_button_label}</button>
                        <button className="h-14 px-10 rounded-full border border-white/20 bg-transparent font-bold hover:bg-white/5 transition-all">{content.home.download_brochure_button_label}</button>
                     </div>
                  </div>
