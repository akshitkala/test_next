      ...
      <main className="relative pt-32 pb-20 md:pt-48 md:pb-32">
        <div className="mx-auto max-w-7xl px-6">
          <div className="mb-16 text-center">
            <h2 className="mb-4 text-sm font-bold uppercase tracking-widest text-accent">{content.features.features_section_small_heading}</h2>
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {features.map((feature, i) => <div key={i} className="group relative rounded-3xl border border-white/5 bg-white/5 p-8 transition-all hover:bg-white/[0.08] hover:translate-y-[-4px]">
                ...
                <div className="mt-6 flex items-center text-sm font-semibold text-accent opacity-0 transition-opacity group-hover:opacity-100">{content.features.feature_card_learn_more_button}<svg className="ml-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="mt-32 rounded-[2.5rem] bg-gradient-to-b from-white/[0.05] to-transparent p-12 text-center border border-white/5">
            <h2 className="mb-6 text-3xl font-bold md:text-4xl">{content.features.cta_section_heading}</h2>
            <p className="mb-10 text-muted-foreground">{content.features.cta_section_description}</p>
            <button className="rounded-full bg-foreground px-8 py-4 font-bold text-background transition-transform hover:scale-105 active:scale-95">{content.features.cta_section_button_text}</button>
          </div>
        </div>
      </main>
