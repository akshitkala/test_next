  ...
  return <nav className="fixed top-0 z-50 w-full border-b border-white/5 glass-morphism">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="text-xl font-bold tracking-tighter">{content.navbar.nexus_title}</span>
        </Link>
        ...
        <button className="rounded-full bg-foreground px-5 py-2 text-sm font-semibold text-background transition-transform hover:scale-105 active:scale-95">{content.navbar.get_started_button}</button>
      </div>
    </nav>;
}